...
REMOVE_DUPLICATES = False   # убирать дубли по reg_num
...
def clean_company_name(name: str) -> str:
    """
    Заменяет длинные формы названий на короткие аббревиатуры.
    Например:
...
def normalize_company(item: dict[str, Any]) -> dict[str, Any]:
    ...
    full_name = clean_company_name(item.get("vnaim", ""))
    short_name = clean_company_name(item.get("vfn", "") or item.get("vn", ""))
...
def main() -> None:
    ...
    output_dir = Path(".")
    output_file = output_dir / "egr_leads.csv"
