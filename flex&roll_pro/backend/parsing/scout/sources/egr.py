...
# Keywords in company names that are clearly irrelevant for label printing
# (строительство, IT, юридические, медицина, транспорт, финансы, etc.)
_IRRELEVANT_KEYWORDS = [
    "строит", "ремонт", "монтаж", "проектир", "архитект", "кровл", "фасад",
    "сантехн", "электромонтаж", "отделк",
    "юридич", "адвокат", "нотариат", "консалтинг", "аудит", "бухгалтер",
    "банк", "страхов", "лизинг", "микрофинанс", "кредит",
    "медицин", "стоматол", "клиник", "аптек", "ветерин", "фармац",
    "транспорт", "логистик", "перевоз", "такси", "экспедиц",
    "недвижим", "риелтор", "аренд",
    "программ", "разработк", "it-", " ит ", "цифров", "софтвер", "веб-",
    "охран", "безопасн",
    "образован", "школ", "курсы", "репетитор",
    "салон красот", "парикмахер", "маникюр",
    "туризм", "турагент", "путешеств",
    "рекламн", "маркетинг", "pr-",
]
...
def _is_irrelevant(name: str) -> bool:
    """Return True if the company name contains clearly irrelevant keywords."""
    lower = name.lower()
...
class EGRSource(BaseSource):
    ...
    def fetch_candidates(
        ...
        **kwargs,
        ...
        logger.info(f"[EGR] Fetching {start_date} → {end_date} (по дням)")

        # Fetch day-by-day to avoid API timeouts on large ranges
        seen_ids: set[str] = set()
        current = start_date
        while current <= end_date:
            try:
                raw = self._fetch(current, current)
                logger.info(f"[EGR] {current}: {len(raw)} записей")
            except Exception as e:
                logger.error(f"[EGR] API error for {current}: {e}")
            ...
            current += timedelta(days=1)
            if current <= end_date:
                time.sleep(1.0)  # polite delay between daily requests
    ...
    def _fetch(self, start_date: date, end_date: date) -> list[dict]:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                last_exc = e
                logger.warning(f"[EGR] Attempt {attempt}/{MAX_RETRIES} failed: {e}")
    ...
    def _to_lead(self, item: dict) -> Lead | None:
        ...
        if _is_irrelevant(display_name):
            return None
